...
impl FromStr for Rucksack {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(Rucksack {compartment1: s[0..s.len()/2].to_string(), compartment2: s[s.len()/2..s.len()].to_string()})
    }
}
...
fn main() {
    let input = fs::read_to_string("/home/damian/rust/advofcode2022/adv_3/input.txt").expect("Can't open the file");
    let rucksacks :Vec<_>= input.split("\n")
        .filter(|a| a.len()>0)
        .map(|line| Rucksack::from_str(line).unwrap())
        .collect();
    let sum_part1 : u32= rucksacks.iter()
        .map(|f| f.find_common().unwrap())
        .map(|f| to_priority(f))
        .sum();
    println!("Part 1 sum :{}",sum_part1);
    let sum_part2 :u32= rucksacks.array_chunks::<3>()
        .map(|i| {find_common_badge(i).unwrap()})
        .map(|f| to_priority(f)).sum();
